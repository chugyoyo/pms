...
public class WebAppExceptionAdvice {
    ...
    @ResponseBody
    public R SQLIntegrityConstraintViolationExceptionHandler(Exception ex) {
        log.error("ExceptionMsg:{}, SQLIntegrityConstraintViolationException:{}", ex.getMessage(), ex);
        return R.error("为了保护数据库完整性，您的操作被数据库拒绝！");
    }
