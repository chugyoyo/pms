        ...
        try {
            ...
            if (list != null) {
                ...
                for (Map<String, Object> row : list) {
                    // 统一转驼峰
                    String tableField = (String) row.get("Field");
                    String field = StringUtil.toCamelCase(tableField);
                    String type = GeneratorUtil.getJavaType((String) row.get("Type"));
                    if (!hasBigDecimal && "BigDecimal".equals(type)) {
                        hasBigDecimal = true;
                    }
                    obuf += "    /**\n     * " + MapUtil.getStr(row, "Comment") + "\n     */\n";
                    // Swagger注解
                    obuf += "    @ApiModelProperty(value = \"" + MapUtil.getStr(row, "Comment") + "\", dataType = \"" + type + "\")\n";
